...
Memory::fillRamWithInitPattern()
{
    switch (config.ramInitPattern) {
        ...
        case RamInitPattern::RANDOMIZED:
            ...
            GetRNGstate();
            if (chip) for (isize i = 0; i < config.chipSize; i++) chip[i] = (u8)(unif_rand() * 256);
            if (slow) for (isize i = 0; i < config.slowSize; i++) slow[i] = (u8)(unif_rand() * 256);
            if (fast) for (isize i = 0; i < config.fastSize; i++) fast[i] = (u8)(unif_rand() * 256);
            PutRNGstate();
            break;
